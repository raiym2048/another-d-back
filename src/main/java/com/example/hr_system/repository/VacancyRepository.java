...
public interface VacancyRepository extends JpaRepository<Vacancy, Long> {
    ...
    @Query(value = "SELECT v.* FROM vacancies v " +
            ...
            "WHERE ((LOWER(p.name) LIKE LOWER(CONCAT('%', :vacancyName, '%')) OR " +
            "LOWER(e.company_name) LIKE LOWER(CONCAT('%', :vacancyName, '%'))) OR :vacancyName IS NULL) " +
            "AND (" +
            "CASE " +
            "WHEN DATEDIFF(CURRENT_DATE, v.creation_date) < 2 THEN 'TODAY' " +
            "WHEN DATEDIFF(CURRENT_DATE, v.creation_date) BETWEEN 2 AND 7 THEN 'THIS_WEEK' " +
            "WHEN DATEDIFF(CURRENT_DATE, v.creation_date) BETWEEN 8 AND 29 THEN 'THIS_MONTH' " +
            "ELSE 'THIS_YEAR' " +
            "END) = :dateFilter", nativeQuery = true)
